...
[assembly: MelonInfo(typeof(ExpandRecipe.Main), "ExpandRecipe-Improved", "1.0.2", "robbmanes, Patched by: MethodNotAllowed")]
...
namespace ExpandRecipe
{
	...
	public class Main : MelonMod
	{
		...
		{
			Log = LoggerInstance;
			Log.Msg($"Last tested on Schedule I version \"{this.testedVersion}\"");
		}
		...
		public override void OnSceneWasLoaded(int buildIndex, string sceneName)
		{
			if (sceneName == "Main")
			{
				try
				{
                    var managers = UnityEngine.Object.FindObjectsOfType<ProductManager>();
					if (managers.Count > 0)
                    {
                        Main.productManager = managers[0];
                    }
                    else
                    {
                        LoggerInstance.Warning("No ProductManager found in 'Main' scene.");
                    }
				}
				catch (Exception value)
				{
					...
					LoggerInstance.Error(defaultInterpolatedStringHandler.ToStringAndClear());
				}
			}
		...
		{
			if (Main.productManager == null)
			{
                try {
				    var managers = UnityEngine.Object.FindObjectsOfType<ProductManager>();
                    if (managers.Count > 0) Main.productManager = managers[0];
                } catch { }
			}

            if (Main.productManager == null)
            {
                Main.Log.Error("ProductManager not found, skipping recursion.");
                return new Sys.List<Sys.List<Main.ExpandedItem>>();
            }
			...
			Sys.List<string> visitedIds = new Sys.List<string>();
			// Add the root product ID to visited list to prevent immediate recursion back to start
			visitedIds.Add(productEntry.Definition.ID);
			return Main.GetExpandedRecipesInternal(recipes, visitedIds, 0);
		}

		// Token: 0x06000006 RID: 6 RVA: 0x00002110 File Offset: 0x00000310
		private static Sys.List<Sys.List<Main.ExpandedItem>> GetExpandedRecipesInternal(Il2CppGeneric.List<StationRecipe> baseRecipes, Sys.List<string> visitedIds, int depth = 0)
		{
            if (depth > 15) return new Sys.List<Sys.List<Main.ExpandedItem>>();
							...
							if (ingredient.Item.Category == EItemCategory.Product)
							{
								...
								else
								{
									foreach (StationRecipe stationRecipe2 in productDefinition2.Recipes)
									{
										...
										if (visitedIds.Contains(ingredient.Item.ID))
										{
											// Cycle detected: Treat as leaf node
										...
										// Create new visited path for this branch
										Sys.List<string> newVisited = new Sys.List<string>();
										foreach(string id in visitedIds) newVisited.Add(id);
										newVisited.Add(ingredient.Item.ID);
										...
										foreach (Sys.List<Main.ExpandedItem> list5 in Main.GetExpandedRecipesInternal(list4, newVisited, depth + 1))
										{
		...
		public static void BuildUIWithRecipe(ProductEntry productEntry, Sys.List<Sys.List<Main.ExpandedItem>> expandedRecipes, GameObject recipeTextUI, GameObject recipesContainerUI)
		{
			...
			foreach (Sys.List<Main.ExpandedItem> list2 in expandedRecipes)
			{
				...
				Main.Log.Msg(defaultInterpolatedStringHandler.ToStringAndClear());
				string text = "\n-- Breakdown --";
		...
		public string testedVersion = "0.4.5f1";
